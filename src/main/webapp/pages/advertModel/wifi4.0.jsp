...
<body class="gray-bg bgimage"  id="wifibgId" style="position:relative;">
				<div id="wifi1" style="position:absolute;z-index:10;left:0px;right:0px;top:0px;bottom:0px;">
					<span style="color:write;"><span>
				</div>
			<div class="middle-box text-center loginscreen  animated fadeInDown" style="padding-top:0px;">
			<div style="height: 140px;">
				<h1 class="logo-name" style="display:inline;">
					<i class="fa fa-wifi wificolor" style="margin-bottom:100px;"></i>
				</h1>
			</div>
	...
	<script>
		...
		if(materurlwifi1){
			...
			$("#wifi1").css('background-repeat', "no-repeat");
			$("#wifi1").css('background-size', "cover");
			for(var i=0;i<=tshownum;i++){
				(function(tempi){
					setTimeout(function(){
						var p=tshownum-tempi;
						if(p==0){
							$("#wifi1").fadeOut("slow");
			 				$('#exampleModal').modal();
						}else{
							$("#wifi1 span").html("倒计时"+p+"秒");
						}
					},tempi*1000);
				})(i)
			}
