...
Ext.define('Ext.ux.uploadPanel.UploadPanel',{
	...
	columns : [
		...
		{text: '自定义文件名', width: 130,dataIndex: 'fileName',hidden:true,editor: {xtype: 'textfield'}},
		...
		}},
        {
            ...
            width:50,
            items: [{
                icon: rootUrl+'/components/extjs/src/ux/uploadPanel/icons/delete.gif',
                tooltip: 'Remove',
	...
	post_params : {},
	upload_url : 'test.do',
	flash_url : rootUrl+"/components/extjs/src/ux/swfupload/swfupload.swf",//"/swfupload/swfupload.swf",
	flash9_url :rootUrl+"/components/extjs/src/ux/swfupload/swfupload_fp9.swf", //"swfupload/swfupload_fp9.swf",
	...
	upload_success_handler : function(file, serverData, responseReceived){
		var me = this.settings.custom_settings.scope_handler;		
		var rec = me.store.getById(file.id);
		if(Ext.JSON.decode(serverData).success=='true'){
			me.restore.reload();
	       	rec.set('percent', 100);
			rec.set('status', file.filestatus);			
		}else{
		...
		if (this.swfupload&&this.store.getCount()>0) {
			if (this.swfupload.getStats().files_queued > 0) {
				for(var i =0;i<this.store.getCount();i++){
				      var temp= this.store.getAt(i) //遍历每一行
				      if(/.*[\u4e00-\u9fa5]+.*$/.test(temp.data.name)){
				    	  Ext.Msg.show({
				  			title : '提示',
				  			msg : '上传文件不能含有中文',
				  			width : 180,
